...
def test_function_correctly_converts_dataframe_to_parquet(mock_client):
    ...
    write_df_to_parquet(test_df, 'test_write_parquet', processed_bucket_name)

    retrieved_df = wr.s3.read_parquet(path=f's3://{processed_bucket_name}/test_write_parquet.parquet')
...
def test_function_puts_dataframe_converted_to_parquet_in_processed_bucket(mock_client):
    ...
    assert mock_client.list_objects_v2(Bucket=processed_bucket_name)['Contents'][0]['Key'] == 'test_write_parquet.parquet'
...
def test_function_raises_exception_when_agruments_invalid(mock_client):
    ...
    with pytest.raises(Exception):
        write_df_to_parquet('wrong', processed_bucket_name)
    
    with pytest.raises(Exception):
        write_df_to_parquet('test', 'wrong')