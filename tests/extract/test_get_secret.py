...
def test_should_return_a_dict_with_correct_keys(sm_client):
    ...
    with patch('src.extract.extract.boto3.client') as mock_client:
        mock_client.get_secret_value.return_value = sm_client.get_secret_value(SecretId=AWS_SECRET_DB_CREDENTIALS_NAME)
...
def test_should_return_a_dict_with_correct_table_name_data(sm_client):
    ...
    with patch('src.extract.extract.boto3.client') as mock_client:
        mock_client.get_secret_value.return_value = sm_client.get_secret_value(SecretId=AWS_SECRET_TABLES_NAMES)
